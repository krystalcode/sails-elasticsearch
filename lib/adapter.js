...
module.exports = (function () {
  ...
  var adapter = {
    ...
    /**
     * @Issue(
     *   "Create the index if it doesn't exist before putting its mapping"
     *   type="bug"
     *   priority="normal"
     *   labels="api compliance"
     * )
    ...
    find: function (connectionName, collectionName, options, cb, index) {
      ...
      for (field in options.where) {
        ...
        if (fieldPartsCount == 1) {
          ...
          if (Array.isArray(options.where[field])) {
            termFilter.terms = {};
            termFilter.terms[field] = options.where[field];
          } else {
            // Otherwise, we create a 'term' filter.
            termFilter.term = {};
            termFilter.term[field] = options.where[field];
          }
          ...
          fieldParts.reduce(function(result, value, key, collection) {
            ...
            result[1].nested.filter = termFilter;
            return;
